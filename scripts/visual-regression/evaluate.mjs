...
async function fetchComments() {
  ...
  let all = [];
  for (;;) {
    const batch = await gh('GET', `${api}/repos/${repo}/issues/${pr}/comments?per_page=100&page=${page}`);
...
function parseApprovals(comments) {
  const set = new Set();
...
async function upsert(body, comments) {
  const existing = comments.find((c) => c.body && c.body.includes(MARKER));
  if (existing) {
    const u = await gh('PATCH', `${api}/repos/${repo}/issues/comments/${existing.id}`, { body });
...
const imgUrl = (f) => `${server}/${repo}/raw/${snapSha}/${prefix}/${f}`;
const img = (f, w = 260) => `<img src="${imgUrl(f)}" width="${w}" alt="">`;
const pct = (r) => `${(r * 100).toFixed(2)}%`;
const titleOf = (i) => `${i.title} › ${i.name} · <i>${i.theme}</i>`;
const tableRow = (i) => `| Before | After | Diff |\n| :--: | :--: | :--: |\n| ${img(i.images.before)} | ${img(i.images.after)} | ${img(i.images.diff)} |\n\n`;
...
const isApproved = (i) => approved.has(norm(i.title)) || approved.has(norm(`${i.title} › ${i.name}`));
