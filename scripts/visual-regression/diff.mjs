...
//
//   node diff.mjs --base <dir> --head <dir> --out <dir>
//
...
import { readJson, ensureDir, readPng, writePng, pad, safe, parseArgs } from './lib.mjs';
...
if (!baseDir || !headDir || !outDir) {
  console.error('Usage: diff.mjs --base <dir> --head <dir> --out <dir>');
  process.exit(1);
}
ensureDir(outDir);

const baseM = readJson(path.join(baseDir, 'manifest.json')).filter((m) => m.file);
const headM = readJson(path.join(headDir, 'manifest.json')).filter((m) => m.file);
const key = (m) => `${m.id}__${m.theme}`;
...
const totals = { changed: 0, added: 0, removed: 0, unchanged: 0 };
for (const it of items) totals[it.status]++;
const report = { totals, items, minPixels, threshold };
